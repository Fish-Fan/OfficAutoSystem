            ...
            <table class="table table-striped">
                ...
                <c:forEach items="${applyList}" var="apply">
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${apply.departmentManagerAdvice == 1}">
                                    通过
                                </c:when>
                                <c:when test="${apply.departmentManagerAdvice == 0}">
                                    拒绝
                                </c:when>
                                <c:when test="${apply.departmentManagerAdvice == null}">
                                    未表态
                                </c:when>
                            </c:choose>
                        </td>
...
<script>
    $(function () {
        ...
        $('.confirmBtn').on('click',function () {
            ...
            $.get('/position/boss/advice/yes?id='+ id,function (data) {
                $('.modal').modal('hide');
        ...
        $('.rejectBtn').on('click',function () {
            var thisBtn = $(this);
            var id = thisBtn.attr('attr');
            $.get('/position/boss/advice/reject?id='+id,function (data) {
                console.log(data.status);
                $('.modal').modal('hide');
